...
def get_opt(params, cfgopt):
    ...
    if scheduler_type is not None:
        if scheduler_type == 'exponential':
            ...
            starting_epoch = int(getattr(cfgopt, "starting_epoch", 0))

            def lambda_rule(ep):
                lr_l = 1.0 - min(1, max(0, (ep-starting_epoch) - 0.5 * step_size) / float(0.45 * step_size)) * (1 - 0.01)
                return lr_l
...
def rotation_loss_class_fov(out_rotation_x, angle_x):
 
    length = out_rotation_x.size(-1)

    if out_rotation_x.size(-1) == length:
        criterion = nn.CrossEntropyLoss()
        loss_x = criterion(out_rotation_x, angle_x.long())
    else:
        assert False
    return loss_x
